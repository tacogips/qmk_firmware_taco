# ErgoDash `tacogips-mac` Keymap

- source: [`keymap.c`](/Users/taco/gits/tacogips/qmk_firmware_taco/keyboards/omkbd/ergodash/rev1/keymaps/tacogips-mac/keymap.c)
- layer: `top layer` only
- `1 cell = 1 physical key`
- each cell shows `key` and `m[row,col]`
...
| 4 | `KC_ENT`<br>`m[4,0]` | `KC_BSPC`<br>`m[4,1]` | `KC_DEL`<br>`m[4,2]` | `KC_LGUI`<br>`m[4,3]` | `KC_LGUI`<br>`m[4,4]` | `KC_LCTL`<br>`m[4,5]` | `KC_SPC`<br>`m[4,6]` |  | `KC_LALT`<br>`m[9,6]` | `KC_LSFT`<br>`m[9,5]` | `KC_SPC`<br>`m[9,4]` | `KC_LCTL`<br>`m[9,3]` | `KC_LEFT`<br>`m[9,2]` | `KC_DOWN`<br>`m[9,1]` | `KC_RGHT`<br>`m[9,0]` |
